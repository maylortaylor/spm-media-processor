...
def find_year_folders(root_dir: Path, year_filter: str | None = None) -> list[Path]:
    ...
    folders = sorted(
        d for d in root_dir.iterdir() if d.is_dir() and not d.name.startswith(".") and re.match(r"^\d{4}$", d.name)
    )
    if year_filter:
...
def find_event_folders(year_dir: Path) -> list[Path]:
    ...
    return sorted(d for d in year_dir.iterdir() if d.is_dir() and not d.name.startswith("."))
...
def run_deep_batch(
    ...
    if s["errored"]:
        ...
        print("  Folders needing manual review still have unapproved segments.")
        print('  Run: python process.py review "<folder>" for each one, then re-export.')
