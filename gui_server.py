    ...
    if segments_file:
        ...
        for sf in sorted(workspace.glob('*_segments.json')):
            ...
            if not d.get('auto_approved') and not all(s.get('approved') for s in d.get('segments', [])):
                pending.append(sf)
        seg_path = pending[0] if pending else sorted(workspace.glob('*_segments.json'))[0] if list(workspace.glob('*_segments.json')) else None
...
async def start_scan_all(request: Request):
    ...
    event_folders = [
        d for d in sorted(year_folder.iterdir())
    ...
    def _batch():
        ...
        def _one(folder_str: str):
            ...
            try:
                f = Path(folder_str)
                print(f'\n[{f.name}] Analyzing...')
                run_analyze(f, output_dir=output_dir, gap_db=gap_db, gap_sec=gap_sec, single_band_threshold=single_band_threshold, non_interactive=True)
                done_count += 1
                jobs.get(_batch._job_id).push_event({'type': 'folder_done', 'folder': f.name, 'folder_path': folder_str, 'stage': 'analyze'})
                jobs.get(_batch._job_id).push_event({'type': 'progress', 'done': done_count, 'total': total})
            except Exception as exc:
                jobs.get(_batch._job_id).push_event({'type': 'folder_error', 'folder': Path(folder_str).name, 'error': str(exc)})
            finally:
                sem.release()
